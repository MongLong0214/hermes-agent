...
| `HERMES_ACP_SKIP_ENV_LOAD` | Set by a parent launcher on the Hermes [ACP](../user-guide/features/acp#host-integration) subprocess it spawns. The exact value `1` (surrounding whitespace ignored) skips loading the profile `.env` during ACP startup only, before the first session loads the agent runtime; the first session still loads the profile `.env` with override, replacing launcher-passed values. Honoured by `hermes-acp` and `python -m acp_adapter`, not by `hermes acp` (which loads `.env` before the adapter starts). Any other value keeps the default load. Not a secret. Don't set manually. |
