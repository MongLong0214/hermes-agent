...
def _persist_branch(db, new_key: str, parent_key: str, title: str, history: list, *, source, cwd, profile_name,
    ...
    if not db.create_session_strict(new_key, source=source, model=model, model_config={"_branched_from": parent_key},
                                    parent_session_id=parent_key, cwd=cwd, profile_name=profile_name, user_id=user_id,
                                    system_prompt=parent_prompt or None, title=title, title_source=title_source):
    ...
    with _session_db(session) as db:
        ...
        try:
            ...
            _persist_branch(db, new_key, old_key, title, history, source=source, cwd=_session_cwd(session),
                            ...
                            model=_session_default_model(session), copy_fields=_BRANCH_COPY_FIELDS, compensate=True,
