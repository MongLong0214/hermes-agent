...
from plugins.platforms.telegram.telegram_recovery import (
    await_abandoned_initialize_owners, retain_abandoned_initialize, schedule_in_place_redelivery)
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _record_polling_progress(self, generation: int) -> bool:
        ...
        if self._send_path_degraded and getattr(self, "_running", False) and not self.has_fatal_error:
            ...
            # Same instance recovered, so no reconnect sweep will replay what it refused while degraded.
            schedule_in_place_redelivery(self, generation, _POLLING_GENERATION_CONTEXT)
