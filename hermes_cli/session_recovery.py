...
_SNAPSHOT_PREFIX = "hermes-session-recovery-"
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix=_owner_prefix(_SNAPSHOT_PREFIX), dir=str(work_root))
    try:
...
def inspect_session_database(source_path: Path, *, work_dir: Optional[Path] = None) -> dict[str, Any]:
    ...
    _remove_dead_owner_dirs(work_root, _SNAPSHOT_PREFIX)
...
def _recover_via_lost_and_found(
    *, source: Path, snapshot_source: Path, snapshot_dir: Path, candidate: Path, output: Path,
    ...
    # See #101409.
    plausibility_conn = _connect(candidate, timeout=5.0)
    try:
...
def recover_session_database(
    ...
    # A killed run's snapshot and stage copies can each be as large as the source; drop them before the space check.
    _remove_dead_owner_dirs(work_root, _SNAPSHOT_PREFIX)
