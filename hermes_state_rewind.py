...
class SessionRewindMixin:
    ...
    def rewind_user_turn(
        ...
        clients can address follow-ups by row; the CLI leaves its history shape alone. ``turn_lease_holder``: the
        running turn's own lease, so a turn can retract its own row. Out-of-range / wrong-shape targets raise
        :class:`RewindTargetUnavailableError`."""
