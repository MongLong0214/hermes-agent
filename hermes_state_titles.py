...
class SessionTitlesMixin:
    ...
    def _set_session_title(self, session_id: str, title: str, *, source: str) -> bool:
        ...
        return self._execute_write(lambda conn: self._write_session_title(conn, session_id, title, source)) > 0

    def _write_session_title(self, conn, session_id: str, title: Optional[str], source: str) -> int:
        """The title compare-and-swap on the caller's write transaction (``title`` already sanitized):
        rows updated, 0 when precedence keeps the current title. A conflict raises ValueError, so a
        caller's transaction that also created the row rolls back whole."""
