    ...
    if requested:
        if requested not in _LOCAL_ROUTER_ROLES:
            raise LocalRouterError(
                f"Unknown routing_role={requested!r}; expected one of "
                f"{sorted(_LOCAL_ROUTER_ROLES)}."
            )
        return requested
    return "subagent"
...
def _run_local_router(command: str, value: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise LocalRouterError(f"local router {command} failed: {exc}") from exc
    ...
    if proc.returncode != 0:
        ...
        try:
            ...
            detail = payload.get("reason") or payload.get("error") or detail
            ...
            pass
        raise LocalRouterError(
            f"local router blocked {command} {value!r}: {detail}"
        )
    ...
    if command == "pick":
        try:
            ...
            raise LocalRouterError(
                f"local router returned invalid pick JSON for {value!r}"
            ) from exc
        if payload.get("blocked") or not payload.get("model") or not payload.get("provider"):
            raise LocalRouterError(
                f"local router returned unusable pick for {value!r}: {payload}"
            )
        return payload
    ...
    *,
    ...
    if record_provider is not None:
        try:
            ...
            logger.error(
                "delegate_task spawned child but local router record failed "
                "(provider=%s, routing_role=%s)",
                record_provider,
                routing_role,
                exc_info=True,
            )
